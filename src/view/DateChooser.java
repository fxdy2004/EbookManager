...
public class DateChooser extends JPanel {
	...
	private void initPanel() {
		...
		monthPanel.setBorder(BorderFactory.createLineBorder(ColorUitl.getSavedColor()));
	...
	private class JP1 extends JPanel {
		...
		public JP1() {
			...
			this.setBackground(ColorUitl.getSavedColor());
			initJP1();
		}
	...
	private class MyLabel extends JLabel implements Comparator<MyLabel>, MouseListener, MouseMotionListener {
		...
		protected void paintComponent(Graphics g) {
			if (day == select.get(Calendar.DAY_OF_MONTH) && month == select.get(Calendar.MONTH)) {
				//如果当前日期是选择日期,则高亮显示  
				g.setColor(ColorUitl.getSavedColor());
